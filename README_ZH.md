...
| **询问一次** | 效果或授权确实不明确，或分类器连续失败三次后转人工确认。分类器回答 `ask`、或因为分类器不可用／目标无法安全披露给分类器而无法审查的越权请求，由插件自己发起审批，并同时挂上精确的官方 grant，因此用户仍然只看到一次弹窗——而忽略 sandbox 字段的工具不可能在无人确认的情况下执行。被调用方取消的调用从未被审查过，因此直接拒绝，既不弹窗也不挂 grant。分类器对越权请求回答 `deny` 时仍然只是直接拒绝，不产生任何审批。显式的一次性越权请求不会因为分类器暂时不可用就被拒绝 |
...
凡是需要 Agent 自己无法作出的裁决的拒绝，都会带上一个类别，Agent guidance 与调用后的恢复提示会告诉模型该类别允许什么。分类器拒绝意味着这个效果需要 Agent 尚未持有的授权：只有在确实需要更宽文件系统沙箱时，才可以把**同一个**调用重新发起，并附上精确的一次性 `sandbox_permissions` 越权请求；否则必须请用户在打字消息中授权这个精确动作，绝不能去找等价路径，也不能把更宽沙箱当成它并不提供的授权。被拒绝的越权请求会被告知：没有新的授权就不要重复。hard deny 是单调的，因此会要求模型把该动作交给用户，而不是重试。deterministic deny 与格式非法的 sandbox 请求既不带类别也没有调用后提示：它们是调用自身的问题，只由 Agent guidance 告诉模型改用可见的字面目标重新规划，或纠正那几个 sandbox 字段。被调用方取消的调用根本不算拒绝——它从未被审查，因此没有提示，模型会被告知先询问再重发同一效果。`ask_user_question` 的答案以工具输出返回，因此只是信息，永远不构成授权。
