...
5. Reviewable semantic risk is sent to an independent classifier. It receives the tool name, redacted bounded arguments, workspace root, deterministic reason, high-confidence literal filesystem effects with their pre-execution `existedBefore` state, at most four recent direct-human messages, and—when present—the exact sandbox request. Repository text, tool output, Assistant/Skill/plugin/subagent text, and the model-written justification are not user authority. A truncated classifier answer is refused outright rather than partially trusted.
6. A classifier `allow` runs the ordinary sandboxed call. A classifier `deny` returns a tool denial so the Agent can choose a safer plan without prompting. `ask` reaches the official approval path only for genuine ambiguity. Classifier unavailability, timeout, malformed output, or a missing model route denies the risky action for the first two consecutive failures so the Agent can retry or replan; the third consecutive failure for that Auto Session falls back to one ordinary manual approval instead of trapping the task in an endless deny loop. A successful classifier response resets the counter, and caller cancellation does not increment it — a cancelled call is denied as never reviewed, with its own marker and no class. An explicit one-shot escalation is exempt: reviewer unavailability never denies it, and neither reviewer unavailability nor a classifier `ask` delegates the approval to the tool body, because only some tools implement the official escalation seam and an inert `sandbox_permissions` argument would otherwise carry any classifier-eligible call past the human. Every refusal the Agent must not work around also tells it which recovery its class permits, so a denial is not answered with an equivalent route; a deterministic denial and a malformed sandbox request record no class and remain the call's own re-plan signal.
7. An explicit `sandbox_permissions: danger-full-access` retry is always classified, even when the underlying command would otherwise be routine. One exact new, narrow, reversible outside target may be authorized from clear direct task intent without forcing the user to repeat magic words. Overwriting or deleting pre-existing data requires exact direct-user authority for that effect and target. Whenever a widening is answered with `ask`, or cannot be reviewed because the reviewer is unavailable or the target cannot be shown to it, the plugin raises the human approval itself and arms the same exact grant, so the official seam — where a tool implements one — resolves against that single decision instead of prompting twice, and a tool that simply ignores the sandbox fields cannot run the call with nobody asked. A caller-cancelled widening is not in that set: it is denied without a prompt and without arming a grant. A classifier `deny` is not in that set either: it returns a bare denial with no approval and no grant. When the classifier clears the widening, the armed grant lets the official request resolve without a prompt, which is the documented one-shot path. The approval listener returns `allowed-once` only when live Agent identity, tool name, call id, target mode, and justification all match; the record is consumed once or removed when the tool settles.
...
While Auto is active, a dynamic system context tells the Agent to perform ordinary work inside the sandbox, request an exact one-shot widening only when the task requires it, split unrelated effects, and treat deletion as the highest-risk operation. It prefers a reversible move, backup, or version-control-backed removal unless permanent deletion is explicit. It also names each refusal class, states that an `ask_user_question` answer is information rather than authorization, and forbids reaching the same effect by an equivalent route after a denial; a post-call recovery notice repeats that instruction for the refusal classes the Agent must not work around: an authority-dependent refusal, a refused escalation, a delegated widening, a repeated standing sandbox request, and a monotonic hard denial. This guidance helps planning but is not a security boundary; the deterministic policy and official sandbox remain authoritative.
...
- transient classifier failure denies only the risky action while routine sandboxed work remains available, with one manual-review fallback after three consecutive failures, and never denies an explicit one-shot escalation — it raises that one approval itself;
- every refusal that needs a decision the Agent cannot make for itself is classified for it: an authority-dependent refusal directs it to escalate the exact call only when a wider sandbox is what it needs, or to obtain a typed user authorization instead of seeking an equivalent route; a refused escalation directs it not to repeat itself without new authority; a delegated widening directs it to report to its parent; a repeated standing sandbox request directs it to drop those fields; and a monotonic hard denial directs it to hand the action to the user;
- a deterministic denial and a malformed sandbox request record no class and earn no post-call notice, because the corrected call is their own recovery; a caller-cancelled call is denied as never reviewed, with its own non-escalating marker and no class;
...
The classifier reuses the Session route, so it must state its own controls instead of inheriting the Session's. Harness `resolveCallWithInfo` materializes an adapter's advertised `defaultEffort` whenever the caller omits one — the DeepSeek adapter advertises `high` — and reasoning tokens then share the answer cap, which truncated real classifier calls to `finish_reason: "length"` and failed closed. The request therefore pins an effort (`off` by default) after checking the exact route's advertised efforts, falls back to `off` when the configured id is unavailable, sends none when the route offers neither, and retries once without the pin if an adapter rejects it. The answer cap is deliberately *not* lowered to match the pin: `LlmResolvedModelInfo` cannot express whether a published `off` means an explicit disable or merely omitting the option, and `@deepseek-ai/dsh-llm-pi-ai` documents the latter — a provider whose own default is to think keeps thinking with `off` selected. `classifierMaxOutputTokens` therefore defaults to, and is capped at, the 4096 ceiling; a smaller value is an operator's explicit choice to risk truncation. A truncated response is refused rather than partially trusted: no provenance signal can separate the model's own conclusion from text it merely quoted out of untrusted input.
